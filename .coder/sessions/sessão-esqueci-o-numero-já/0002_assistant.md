To fix the warning about each child in a list having a unique "key" prop, we need to ensure that each element in the list rendered in the `Sidebar` component has a unique key. Specifically, we need to add a unique key to the buttons inside the prompts list.

Let's update the `Sidebar.tsx` file to include the unique key prop for each button in the prompts list.
