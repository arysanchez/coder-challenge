Para corrigir o erro 422 na rota `/create-prompt`, precisamos garantir que o modelo de dados que estamos recebendo e processando está correto. Vamos ajustar a rota para garantir que estamos recebendo e processando os dados corretamente.
...
async def create_external_prompt(request: List[PromptResponse]):
    headers = {
...
Com essa atualização, a rota `/create-prompt` agora aceita uma lista de `PromptResponse` e processa corretamente os prompts recebidos. Certifique-se de que o frontend está enviando os dados no formato correto.