O problema ocorre porque a rota `/create-prompt` espera uma lista de `CreatePromptRequest`, mas o frontend está enviando uma lista de objetos que não correspondem ao modelo esperado. Vamos ajustar o modelo de dados no backend para aceitar a estrutura correta.
...
Com essa atualização, a rota `/create-prompt` agora aceita uma lista de `PromptResponse` e processa corretamente os prompts recebidos.